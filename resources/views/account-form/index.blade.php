...
@endif
        ...
        <table class="table ">
          ...
          <tbody>
             ...
             @forelse ($acct_approves as $accountOpen)
              <tr class="info">

              <td scope="row">{{ ++$loop->index }}</th>
              <td>{{ $accountOpen->first_name }} {{ $accountOpen->middle_name }} {{ $accountOpen->last_name }}</td>
              <td>{{ $accountOpen->email }}</td>
              <td>{{ $accountOpen->phone }}</td>
              <td>{{ $accountOpen->gender }}</td>
              
               <td>
                    
                <form method="POST" action="{{ route('form.destroy', $accountOpen->id) }}"   enctype="multipart/form-data" >
                 
                       
                       <li class="list-inline-item">
                     <a class="btn btn-success btn-sm rounded-0" href="{{ route('form.show', $accountOpen->id) }} " type="button" data-toggle="tooltip" data-placement="top" title="view"><i class="fa fa-eye">view</i></a>
                      </li>
